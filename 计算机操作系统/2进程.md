...
## 进程间通信
#### 背景：每个进程各自有用户地址空间，任何一个进程的全局变量在另一个进程都看不到，所以进程间通信必须通过内核，在内核开辟出一个缓冲区，进程1把数据从用户空间拷贝到内核缓冲区，进程2再从内核缓冲区把数据读走，内核提供的这种机制称为进程间通信；
#### 进程间通信的7种方式：
#### 匿名管道（PIPE） & 命名管道（FIFO）：
> 匿名管道：存在于具有亲缘关系的进程，单向传输，只存在于内存中；
> 命名管道：存在于任意进程，先进先出，名字存在于文件系统中，内容存放于内存中；
#### 共享内存
#### 消息队列
#### 信号量   
#### socket：不仅可以满足本机进程通信，还能实现远程进程通信；
