...
#### 中断处理程序：在中断程序完成自己的工作之后，使先前被阻塞的设备驱动程序继续运行，

#### 设备驱动程序：每一个连接到计算机上的IO设备，其厂商都需要提供对应的设备驱动程序，
> 大多数系统都将驱动程序归类为：块设备和字符设备；
> 块设备：包含多个可独立寻址的数据块；
> 字符设备：生成或接收字符流；
> 大多数操作系统都定义了一个所有块设备都必须支持的标准接口，以及一个所有字符设备都必须支持的标准接口；

![](../resource/操作系统/设备驱动程序示意图.png)
...
## 盘
## 时钟
#### 常称为定时器，负责维护时间，防止一个进程垄断CPU；
### 时钟硬件
#### 时钟制作：把一块石英晶体做适当的切割，放置在电压下，会产生非常精密的周期性信号，具体的频率与石英晶体有关；
![](../resource/操作系统/时钟.png)
### 时钟软件：
